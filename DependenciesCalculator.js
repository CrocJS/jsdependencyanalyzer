...
DependenciesCalculator.prototype = {
    ...
    __addSymbolStruct: function(symbolStruct) {
        ...
        var processDependencies = function(file, dependencies) {
            var ignoreSymbols = !file ? {} :
                _(symbolStruct.ignore[file])
                    .map(function(x) { this.__getSymbolStruct(x); }, this)
                    .indexBy('id').value();
            
            _.forOwn(dependencies, function(desc, symbol) {
                var depType = typeof desc === 'string' ? desc : desc.depType;
                var meta = typeof desc !== 'string' && desc.meta;
                var curSymbolStruct = this.__getSymbolStruct(symbol, depType.indexOf('Optional') !== -1, meta);
     ...
     * @private
     */
    __getSymbolStruct: function(symbolOrFile, optional, meta) {
